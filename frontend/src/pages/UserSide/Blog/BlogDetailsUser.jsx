...
const BlogDetailsUser = () => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const response = await PublicAxios.get('/user/blog/blogdetails', {
          params: { blogId: id },
        });
  ...
  const handleLikeToggle = async (id) => {
    try {
      ...
      const response = await PublicAxios.get('/blog/like', {
        params: {
          blogId: id,
          userId: userId,
          like: !userLiked // Use the opposite of the current state
        }
      });

      setLikeCount(response.data);
    } catch (error) {
      // If an error occurs, rollback the UI update
      setUserLiked((prevLike) => !prevLike);
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center mb-4">
              {blogData.author.image ? (
                <img
                  src={`${Image_URL}${blogData.author.image}`}
              ...
              <span className="text-gray-600">{blogData.author?.username}</span>
            </div>
            <p className="text-gray-700 mb-4">{getTimeDifference(blogData.created_at)}</p>
            {blogData.image && (
              <img
            ...
            <div className="flex items-center flex-wrap">
              ...
              <span className="text-gray-600 block w-full">{likeCount} likes</span>
            </div>
            <p className="text-gray-800 leading-relaxed mb-8">{blogData.content}</p>
            <div className="mt-8">
              {comment && (
                <div>
