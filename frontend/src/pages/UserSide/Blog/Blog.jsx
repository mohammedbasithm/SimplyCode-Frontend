...
const Blog = () => {
  ...
  const currentPosts = blogPost.slice(firstPostIndex, lastPostIndex);

  useEffect(() => {
    const fetchBlogPost = async () => {
      try {

        const response = await PublicAxios.get('/user/blog');
  ...
  return (
    <>
      <Navigation />
      <div className="pt-14 container mx-auto py-8 flex flex-wrap justify-center">
        <div className='bg-indigo-100 h-64 w-full flex justify-center items-center' style={{ borderBottomRightRadius: '75px' }}>
          <div className='text-blue-900 text-4xl font-bold'>Blog</div>
        </div>
          ...
          <div key={index} className='p-4'>
            <div className="w-64 h-96 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex flex-col">
              ...
              <div className="flex-grow p-5">
                ...
                <button onClick={() => navigate(`/blogdetails/${post.id}`)} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                  Read more
