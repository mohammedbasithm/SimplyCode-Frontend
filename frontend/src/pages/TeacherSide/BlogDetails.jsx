...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await PublicAxios.get('/teacher/blogdetails', {
...
}, [userId, id,successEdit]);
useEffect(() => {
  if (blogData.length > 0) {
    const imagePath =  blogData[0].image
    const slicedImagePath = imagePath.split("/media/")[1];
    setEditInput({
      title: blogData[0].title,
      content: blogData[0].content,
      thumbnail:slicedImagePath,
      blog_id:id,
      teacher_id:userId
    });
  }
  ...
  const handleCommentSubmit = async (event) => {
    ...
    try {
      const response = await PublicAxios.post(
        '/blog/addcomment',
        { blog_id: id, user_id: userId, inputComment },
        {
          headers: {
  ...
  const handleLikeToggle = async (id) => {
    setUserLiked((prevLike) => !prevLike)
    fetchLike(id)
  };
  const handleComment = () => {
  ...
  useEffect(() => {
    const handleComment = async () => {
      ...
      try {
        ...
        if (comment) {
          const response = await PublicAxios.get('/blog/fetchcomment', {
            params: { blogId: id }
          });
        ...
        console.log('fetch comment data faild');
      }
    }
    handleComment();
    setSuccessEdit(false);
  ...
  const blogDelete = async () => {
    try {
      ...
      const response = await PublicAxios.get('/blogdelete', {
        params: {
          blog_id: id,
          user_id: userId
        }
 ...
 const handleCreate=async(event)=>{
  event.preventDefault();
  try{
    const response=await PublicAxios.post('/teacher/editblog-details',editInput,{
      headers:{
 ...
 console.log('editBlog:',editInput);
        ...
        {blogData.length > 0 && (
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-3xl font-bold mb-4">{blogData[0].title}</h1>
              <div className="flex items-center">
                <MdDelete onClick={blogDelete} color='red' fontSize='2em' className="mr-2" />
                <FaEdit onClick={blogEdit} color='blue' fontSize='2em' />
              </div>
            </div>
            <div className="flex items-center mb-4">
              {blogData[0].author.image ? (
                <img
                  src={`${Image_URL}${blogData[0].author.image}`}
                  alt={blogData[0].author?.username}
                  className="w-10 h-10 rounded-full mr-2"
                />
              ) : (
                <img src={image} className="w-10 h-10 rounded-full mr-2" />
              )}
              <span className="text-gray-600">{blogData[0].author?.username}</span>
            </div>
            <p className="text-gray-700 mb-4">{getTimeDifference(blogData[0].created_at)}</p>
            {blogData[0].image && (
              <img
                src={`${Image_URL}${blogData[0].image}`}
            ...
            <div className="flex items-center flex-wrap">
              <div className="flex items-center">
                {userLiked ? (
                  <FaHeart onClick={() => handleLikeToggle(blogData.id)} className="mr-10" color='red' fontSize='1.5em' /> // Added margin to the right

                ) : (
                  <CiHeart onClick={() => handleLikeToggle(blogData.id)} className="mr-10" fontSize='2em' /> // Added margin to the right
                )}
                <FaRegComment onClick={() => handleComment(blogData.id)} className="mr-10" fontSize='1.5em' />
              </div>
              <span className="text-gray-600 block w-full">{blogData.likes?.count} likes</span>
            </div>
            <p className="text-gray-800 leading-relaxed mb-8">{blogData[0].content}</p>

            <div className="mt-8">
              {comment && (
                <div>

                  <h2 className="text-2xl font-bold mb-4" >Comments({commentData.length})</h2>

                  <form onSubmit={(event) => handleCommentSubmit(event)} className="mb-8">
                    <textarea
                      name="content"
                      rows="3"
                    ...
                    <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                      Add Comment {loading && <Spinner />}
                    </button>
                  </form>
                </div>
              )}
              {comment && commentData.map((comment, index) => (
