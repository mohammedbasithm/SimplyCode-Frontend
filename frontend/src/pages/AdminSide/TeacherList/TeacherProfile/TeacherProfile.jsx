...
const TeacherProfile=()=> {
   ...
   useEffect(()=>{
    const teacherData=async()=>{
      try{
        const response=await PublicAxios.get(`/admin/teacherdetails?id=${id}`,{
          headers:{
        ...
        <div className='w-full md:w-3/4 h-full flex flex-col gap-4 '>
          <div className='h-1/2 flex flex-col relative'>
            ...
            <div className='bg-white h-2/3 rounded-bl-md rounded-br-md'>
              ...
              <div className='h-1/4 pl-5 text-2xl font-semibold flex items-center'>{data.username}</div>
              <div className='h-2/4 flex flex-col md:flex-row pl-5'>
                <div className='w-1/2 h-full flex items-center'>
                  <ProfileIcon Icon={<FaMailBulk/>} title='email' subtitle={data.email} />
                </div>
                <div className='w-1/2 h-full flex items-center'>
                  <ProfileIcon Icon={<FaPhone/>} title='phone' subtitle={data.phone} />
                </div>
              </div>
            </div>
